...
in
{
  programs.i3blocks = {
    ...
    bars = {
      top = {
	"1-disk" = {
	  command = "echo \"<span foreground='${blue-2}' background='${bg-1}' size='x-large'> </span><span background='${bg-1}'><span background='${blue-2}' foreground='${grey-1}' size='large' rise='2pt'>  </span><span background='${blue-2}' foreground='${fg-1}' size='small' rise='2pt'> / </span><span background='${blue-2}' size='small' rise='2pt'>$(df -h | grep \"/$\" | awk '{print $5}') </span></span>\"";
	  markup = "pango";
	  interval = 30;
